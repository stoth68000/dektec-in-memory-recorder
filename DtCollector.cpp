//*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#* DtCollector.cpp *#*#*#*#*#*#*#*# (C) 2000-2017 DekTec
//
...
static int ltn_pop(struct app_context_s *ctx, struct node_s **node);
static int ltn_push(struct app_context_s *ctx, uint8_t *buf, int sizeBytes);

static int g_flushToDisk = 0;
static void signal_handler(int signum)
{
	if (signum == SIGUSR2) {
		printf("%s(SIGUSR2)\n", __func__);

		/* Free current buffers to disk. While we're doing this
		 * we can't keep collecting new buffers, so capture will
		 * resume when write to disk has finished.
		 */
		g_flushToDisk = 1;
	}
}
...
{
	struct app_context_s *ctx = (struct app_context_s *)p;
	ctx->threadRunning = 1;
	...
	while (!ctx->threadTerminate) {
		...
		if (flushTime == 0) {
			flushTime = now + 5;
			printf("Preparing to flush in a few seconds...\n");
			continue;
		}
		...
		char fn[256];
		sprintf(fn, "%s-%04d%02d%02d-%02d%02d%02d.raw",
			ctx->filename, 
			lt->tm_year + 1900,
			lt->tm_mon + 1,
			lt->tm_mday,
		...
		fprintf(stderr, "recording   starts -- %s -> %s\n", ctime(&now), fn);

		ctx->fh = fopen(fn, "wb");
		if (!ctx->fh) {
			fprintf(stderr, "Unable to create SDI recording (%s).\n", fn);
			exit(1);
		}

		/* Pop a used node, write to disk. */
		struct node_s *n;
		while (ltn_pop(ctx, &n) == 0) {
			fwrite(n->ptr, 1, n->usedlen, ctx->fh);

			pthread_mutex_lock(&ctx->listMutex);
			xorg_list_append(&n->list, &ctx->listFree);
			pthread_mutex_unlock(&ctx->listMutex);
		}

		time(&now);
		fprintf(stderr, "recording complete -- %s", ctime(&now));
		fclose(ctx->fh);
	...
	ctx->threadComplete = 1;
	ctx->threadRunning = 0;
	pthread_exit(0);
}

static int ltn_init(struct app_context_s *ctx)
{
	xorg_list_init(&ctx->listFree);
	xorg_list_init(&ctx->listUsed);

	pthread_mutex_init(&ctx->listMutex, NULL);

	strcpy(ctx->filename, "dtu351.bin");

	for (int i = 0; i < LTN_LIST_ITEMS; i++) {
	...
	pthread_create(&ctx->threadId, 0, _threadfunc, ctx);
}
...
static int ltn_pop(struct app_context_s *ctx, struct node_s **node)
{
	...
	if (!xorg_list_is_empty(&ctx->listUsed)) {
		struct node_s *n = xorg_list_first_entry(&ctx->listUsed, struct node_s, list);
...
/* Push the current frame to the list.
 * If no free list item exists then pull the oldest item from
 * the used list. This was we retail the latest LTN_NODE_LIST items
 * perpetually.
 */
static int ltn_push(struct app_context_s *ctx, uint8_t *buf, int sizeBytes)
{
	...
	if (!xorg_list_is_empty(&ctx->listFree)) {
		/* For the first N seconds we'll be using any free buffers. */
		n = xorg_list_first_entry(&ctx->listFree, struct node_s, list);
		xorg_list_del(&n->list);
	}

	if (!n && !xorg_list_is_empty(&ctx->listUsed)) {
		/* No 'Free' buffers, recycle a used buffer. */
		n = xorg_list_first_entry(&ctx->listUsed, struct node_s, list);
		xorg_list_del(&n->list);
	}

	if (!n) {
		/* No buffers at all. This should only happen when we've been signalled
		 * to flush all buffers to disk.
		 */
		pthread_mutex_unlock(&ctx->listMutex);
		return sizeBytes;
	}
	...
	if (sizeBytes > n->maxlen) {
		fprintf(stderr, "node too small, needed %d (truncated)\n", sizeBytes);
		written = n->maxlen;
		n->truncated = 1;
	} else
		n->truncated = 0;

	memcpy(n->ptr, buf, written);
	n->usedlen = written;

	xorg_list_append(&n->list, &ctx->listUsed);
...
const char c_QamSymbolRateCmdLineFlag[]     = "ms";
...
const char c_ErrSymbolRateNotSet[]      = "Symbol rate must be set for DTU-236 in QAM-A/C";
...
void CommandLineParams::Init()
{
    ...
    m_QamSymbolRate = DTAPI_MOD_SYMRATE_AUTO;
...
void CommandLineParams::ParseCommandLine(int argc, char* argv[])
{
    ...
    {
        if (m_ModType != DTAPI_MOD_DVBS2_QPSK && m_ModType != DTAPI_MOD_DVBS2_8PSK 
            && m_ModType != DTAPI_MOD_DVBS2_16APSK && m_ModType != DTAPI_MOD_DVBS2_32APSK)
...
void  CommandLineParams::ParseParamFlag(char* pParam, bool First, bool Last)
{
    ...
    if ( 0==strcmp(pParam, c_DvcTypeCmdLineFlag) )
        ...
        m_LastFlagReqsArg = true;
    else if ( 0==strcmp(pParam, c_QamSymbolRateCmdLineFlag) )
        m_LastFlagReqsArg = true;
...
void  CommandLineParams::ParseParamNotFlag(char* pParam, bool First, bool Last)
{
    ...
    else if ( 0==strcmp(m_pLastFlag, c_QamSymbolRateCmdLineFlag) )
    {
        m_QamSymbolRate = atoi(pParam);
        if (m_QamSymbolRate<5000000 || m_QamSymbolRate>7100000)
            throw Exc(c_CleInvalidArgument, m_pLastFlag);
    } 
...
const char* CommandLineParams::RxMode2Str() const
{
    ...
    if ( m_RxMode & DTAPI_RXMODE_SDI )
    {
        ...
        else if ( (m_RxMode&DTAPI_RXMODE_SDI_MASK) == DTAPI_RXMODE_SDI_ACTVID )
        {
            if ( 0!=(m_RxMode & DTAPI_RXMODE_SDI_10B ) )
...
void Recorder::AttachToInput()
{
    ...
    {
        // Special case: driver version is not compatible with this version of DtCollector,
    ...
    // Check for IP output
    m_Ip =  (m_DtInp.m_HwFuncDesc.m_Flags&DTAPI_CAP_IP) !=0;
}
...
void Recorder::DisplayPlayInfo()
{
    ...
    if ( m_Demod )
    {
            ...
            || m_CmdLineParams.m_ModType==DTAPI_MOD_QAM256 )
        {
            ...
            if (m_CmdLineParams.m_QamSymbolRate == DTAPI_MOD_SYMRATE_AUTO)
                LogF("- Symbol rate           : auto");
            else
                LogF("- Symbol rate           : %d", m_CmdLineParams.m_QamSymbolRate);
        }  
        ...
        if ( m_CmdLineParams.m_ModType == DTAPI_MOD_IQDIRECT )
        {
            ...
            LogF("- Bandwitdh             : %d", m_CmdLineParams.m_IqBandwidth );
...
void Recorder::InitInput()
{
    ...
    if ( m_Demod )
    {
        ...
        if (    m_CmdLineParams.m_ModType==DTAPI_MOD_QAM4
             || m_CmdLineParams.m_ModType==DTAPI_MOD_QAM16
             || m_CmdLineParams.m_ModType==DTAPI_MOD_QAM32
             || m_CmdLineParams.m_ModType==DTAPI_MOD_QAM64
             || m_CmdLineParams.m_ModType==DTAPI_MOD_QAM128
...
void Recorder::RecordFile()
{
    ...
    while ( !_kbhit() && !ValidSignal) {
        ...
        else if (m_CmdLineParams.m_RecDtSdiFile)
        {
...
#if LTN
    printf("\nUse CTRL-C to abort\n");
    printf("Send pid %d a SIGUSR2 to dump the current buffers to disk.\n", getpid());
    printf("Eg. kill -s SIGUSR2 %d\n", getpid());
#endif
    // For SDI mode we need to add a dtsdi file header
    if ( m_CmdLineParams.m_RecDtSdiFile )
    {
        // Create header
        FileHdr.m_BaseHdr.m_MagicCode1 = DTSDI_MAGIC_CODE1;
        FileHdr.m_BaseHdr.m_MagicCode2 = DTSDI_MAGIC_CODE2;
        FileHdr.m_BaseHdr.m_MagicCode3 = DTSDI_MAGIC_CODE3;
        FileHdr.m_BaseHdr.m_FmtVersion = DTSDI_FMT_VERSION2;
        
        // Set data type in header
        FileHdr.m_BaseHdr.m_DataType = VidStd2DtSdiDataType(SdiSubValue);
        
        // Add flags
        FileHdr.m_BaseHdr.m_Flags = 0;
        // FULL or active video mode?
        if ( (m_CmdLineParams.m_RxMode&DTAPI_RXMODE_SDI_MASK) == DTAPI_RXMODE_SDI_FULL )
            FileHdr.m_BaseHdr.m_Flags = DTSDI_SDI_FULL;
        else
            FileHdr.m_BaseHdr.m_Flags = DTSDI_SDI_ACTVID;
        // 8-, 10- or 16-bit?
        if ( 0!=(m_CmdLineParams.m_RxMode & DTAPI_RXMODE_SDI_10B) )
            FileHdr.m_BaseHdr.m_Flags |= DTSDI_SDI_10B;
        else if ( 0!=(m_CmdLineParams.m_RxMode & DTAPI_RXMODE_SDI_16B) )
            FileHdr.m_BaseHdr.m_Flags |= DTSDI_SDI_16B;
        // Huffman compressed?
        if ( DTAPI_RXMODE_SDI_HUFFMAN==(m_CmdLineParams.m_RxMode & DTAPI_RXMODE_SDI_HUFFMAN) )
            FileHdr.m_BaseHdr.m_Flags |= DTSDI_SDI_HUFFMAN;

        FileHdr.m_FrameSize = -1;   // fill in later
        FileHdr.m_NumFrames = 0;    // fill proper value later

        // Write header to file
#if LTN
	int NumBytesWritten = ltn_push(&ctx, (uint8_t *)&FileHdr, sizeof(FileHdr));
#else
    ...
    while (1 || !_kbhit())
    {
        ...
        if ( m_CmdLineParams.m_RecDtSdiFile )
        {
            ...
            FileHdr.m_NumFrames++;
            // Set size once
            if (FileHdr.m_FrameSize == -1)
                FileHdr.m_FrameSize = NumToRead;
        }
        else
        {
...
#if LTN
        static int stopWriting = 0;
        if (!stopWriting) {
            int wlen = ltn_push(&ctx, (uint8_t *)m_pBuf, NumToWrite);
            if (NumToWrite != wlen) {
                printf("Requested to write %d bytes, wrote %d\n", NumToWrite, wlen);
                stopWriting = 1;
...
int Recorder::Record(int argc, char* argv[])
{
    ...
    try
    {
        ...
        LogF("DtCollector - Recording Utility V%d.%d.%d (c) 2000-2017 DekTec Digital Video B.V.\n",
...
void Recorder::ShowHelp()
{
    Log("" );
    Log("Usage:");
    Log( "   DtCollector recfile [-t type] [-n number] [-i port] [-x maxsize]\n" \
    ...
    Log("              SDI16B        Store complete SDI frame, in 16-bit format");
    ...
    Log("               7   (for 7MHz)");
    Log("               8   (for 8MHz)");
    Log("   -ms  Symbol rate for J.83 Annex * (default: autodetect)");
    ...
    Log("");
    Log("Examples:");
    Log("   DtCollector myfile.ts -x 10");
    Log("   DtCollector myfile.ts -t 124 -i 2 -m ST204");
    Log("   DtCollector mysdifile.dtsdi -t 245 -m SDI10B -c ON");
    Log("Example recording the first TS in a MIS DVBS2:");
    Log("   DtCollector DVBS2.ts -t 2137 -i 1 -mt DVBS2_QPSK -mf 950 -m ST188");
    Log("Examples recording L3 baseband frames:");
    Log("   DtCollector DVBS2.ts -t 2137 -i 1 -mt DVBS2_QPSK -mf 950 -m STL3");
    Log("   DtCollector DVBS2.ts -t 2137 -i 1 -mt DVBS2_QPSK -mf 950 -m STL3TS");
    Log("");
    LogF("DtCollector version: %d.%d.%d\n", DTRECORD_VERSION_MAJOR, DTRECORD_VERSION_MINOR,
